function MOD_FreeSurf2D
    ...
    InfFile = 0;
    TotTime = double(0.0);
    % Get our input HDF5 filename and check for existence.
    [file,path] = uigetfile('*.h5');
    H5FileP = fullfile(path,file);
    if not( isfile( H5FileP ) )
        fprintf('File %s not found!!!', H5FileP);
        return;
    end
    % make sure current directory is where the input file is
    oldFolder = cd(path);
    % Declare global variables - should save memory.
    % modified to use HDF5 file for input and output
    rEADh5iNPUT(H5FileP);
    ...
    fprintf(InfFile,'Total Elapsed time in min. for the simulation is: %20.4f\n',...
       (TotalTime/60));
    fclose(InfFile);
    fprintf('Successful MOD_FreeSurf2D simulation!!!\n');
    fprintf('Total Elapsed time is: %20.4f min\n', (TotalTime/60));    
    % not needed
